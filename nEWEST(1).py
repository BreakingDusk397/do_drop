...
def get_time_til_close(symbol):
    ...
    try:
        ...
        print("get_time_til_close error.")
        print(traceback.format_exc())
        pass
...
def get_inventory_risk(symbol):
    ...
    try:
        ...
        if symbol == "MSFT":
            inventory_risk = 0.0000002 * abs(inventory_qty)
...
def take_profit_method(symbol):
    try:
        ...
        if float(ORDERS[1][10]) / abs(float(ORDERS[1][6])) >=  0.03:
            cancel_orders_for_symbol(symbol=symbol)
            time.sleep(1)
            trading_client.close_position(symbol)
...
def limit_order(symbol, spread, side, take_profit_multiplier, loss_stop_multiplier, loss_limit_multiplier, qty, inventory_risk):
    ...
    end_of_day = datetime(now.year, now.month, now.day, hour=15)
...
def make_model(dataset, symbol, side):
        ...
        #cancel_orders_for_symbol(symbol="MSFT")
        match_orders_for_symbol(symbol=symbol)
        #cancel_orders_for_symbol(symbol="AAPL")
        #match_orders_for_symbol(symbol="AAPL")
        ...
        dataset['spread1'] = dataset['Open1'] - ((dataset['Low1'] + dataset['High1'])/2)
        dataset['spread12'] = dataset['high1'] - dataset['low1']
        dataset['Volatility1'] = (np.log(dataset['Open1']).ewm(span=5).std() * np.sqrt(5))
        dataset['Volatility12'] = (np.log(dataset['Volatility1']).ewm(span=5).std() * np.sqrt(5))


        cols_features = ['last_return', 'std_normalized', 'ma_ratio', 'price_deviation', 'volume_deviation', 'OBV',]
        ...
        dataset['last_return1'] = np.log(dataset['Open1']).pct_change()
        dataset['std_normalized1'] = np.log(dataset['Open1']).rolling(std_period).apply(std_normalized)
        dataset['ma_ratio1'] = np.log(dataset['Open1']).rolling(ma_period).apply(ma_ratio)
        dataset['price_deviation1'] = np.log(dataset['Open1']).rolling(price_deviation_period).apply(values_deviation)
        #dataset['volume_deviation1'] = np.log(dataset['volume1']).rolling(volume_deviation_period).apply(values_deviation)
        dataset['OBV1'] = stats.zscore((np.sign(dataset['Open1'].diff()) * dataset['volume1']).fillna(0).cumsum())

        dataset['ratio'] = z_score(dataset["Open"]) / z_score(dataset["Open1"])
        dataset['ratio_reversed'] = z_score(dataset["Open1"]) / z_score(dataset["Open"])

        dataset['difference'] = z_score(dataset["Open"]) - z_score(dataset["Open1"])
        dataset['difference_reversed'] = z_score(dataset["Open1"]) - z_score(dataset["Open"])

        dataset['ratio_v'] = z_score(dataset["Volatility"]) / z_score(dataset["Volatility1"])
        dataset['ratio_reversed_v'] = z_score(dataset["Volatility1"]) / z_score(dataset["Volatility"])

        dataset['difference_v'] = z_score(dataset["Volatility"]) - z_score(dataset["Volatility1"])
        dataset['difference_reversed_v'] = z_score(dataset["Volatility1"]) - z_score(dataset["Volatility"])
        sos = butter(4, 0.125, output='sos')

        for i in dataset.columns.tolist():
            dataset[str(i)+'_detrend'] = detrend(dataset[i])
        ...
        if int(now1.hour) == 14:
            if int(now1.minute) == 56:
                ...
                dataset_AAPL.to_csv(f'/home/vboxuser/Documents/dataset_AAPL_{now1}.csv')
                y = pd.DataFrame(y)
        ...
        print('\n y: \n', y)
        print('\n y_sell: ', y_sell)
        #catboost_class_buy = make_model(dataset[-(len(y)):], y, symbol="MSFT", side="buy")
        ...
        #catboost_class_sell = make_model(dataset[-(len(y_sell)):], y_sell, symbol="MSFT", side="sell")
        ...
        if my_file.exists():
            ...
            """
        selected_features = catboost_class.select_features(train_dataset, eval_set=test_dataset, features_for_select=list(dataset.columns), num_features_to_select=10, steps=5, algorithm='RecursiveByShapValues', train_final_model=True)
        ...
        grid = {
            ...
            'fold_len_multiplier':np.linspace(2, 5, 50),
...
async def trade_data_handler(data):
    ...
    if symbol == "MSFT":
        ...
        best_bid, best_ask, midpoint, df1, inventory_qty = get_orderbook("MSFT")

        d = {'close':[ask_price],'volume':[volume], 'Open':float(df1['Open'][-1]), 'High':[df1['High'][-1]], 'Low':[df1['Low'][-1]], 'Close':[df1['Close'][-1]], 
                'Volume':[df1['Volume'][-1]],
                'mu':[df1['mu'][-1]], 'gamma':[df1['gamma'][-1]], 'sigma':[df1['sigma'][-1]], 'k':[df1['k'][-1]],
                'bid_alpha':[df1['bid_alpha'][-1]], 'ask_alpha':[df1['ask_alpha'][-1]], 'ask_sum_delta_vol':[df1['ask_sum_delta_vol'][-1]], 
                'bid_sum_delta_vol':[df1['bid_sum_delta_vol'][-1]], 
                'bid_spread_aysm':[df1['bid_spread_aysm'][-1]], 
                'ask_spread_aysm':[df1['ask_spread_aysm'][-1]], 'inventory':[df1['inventory'][-1]],
                'bid_spread_aysm2':[df1['bid_spread_aysm2'][-1]], 
                'ask_spread_aysm2':[df1['ask_spread_aysm2'][-1]],
                'midpoint_MSFT':[midpoint],
                }
        ...
        ask_price_list3 = pd.merge(left=ask_price_list3, right=volume, left_index=True, right_index=True)
        for i in ['Open', 'High','Low','Close', 'Volume', 'inventory' ,'mu','gamma','sigma','k', 'bid_alpha', 'ask_alpha', 'ask_sum_delta_vol', 'bid_sum_delta_vol', 'bid_spread_aysm', 'ask_spread_aysm', 'bid_spread_aysm2', 'ask_spread_aysm2', 'midpoint_MSFT']:
            ask_price_list_temp = ask_price_list[i].resample('10S').mean()
            ask_price_list3 = pd.merge(left=ask_price_list3, right=ask_price_list_temp, left_index=True, right_index=True)
    ...
    if symbol == 'AAPL':
        ...
        best_bid, best_ask, midpoint_AAPL, df2, inventory_qty = get_orderbook("AAPL")

        d2 = {'close':[ask_price2],'volume':[volume2], 'Open_AAPL':[df2['Open'][-1]], 'High_AAPL':[df2['High'][-1]], 'Low_AAPL':[df2['Low'][-1]], 'Close_AAPL':[df2['Close'][-1]],'midpoint_AAPL':[midpoint_AAPL],}
        ...
        ask_price_list2 = pd.merge(left=ask_price_list2, right=volume2, left_index=True, right_index=True)

        for i in ['Open_AAPL', 'High_AAPL','Low_AAPL','Close_AAPL', 'midpoint_AAPL']:
            ask_price_list_temp = ask_price_list5[i].resample('10S').mean()
            ask_price_list2 = pd.merge(left=ask_price_list2, right=ask_price_list_temp, left_index=True, right_index=True)
    ...
    if symbol == 'TSLA':
        ...
        best_bid, best_ask, midpoint_TSLA, df2, inventory_qty = get_orderbook("TSLA")

        d2_TSLA = {'close':[ask_price2],'volume':[volume2], 'Open_TSLA':[df2['Open'][-1]], 'High_TSLA':[df2['High'][-1]], 'Low_TSLA':[df2['Low'][-1]], 'Close_TSLA':[df2['Close'][-1]],'midpoint_TSLA':[midpoint_TSLA]}
        row2_TSLA = pd.DataFrame(d2_TSLA, index = [timestamp2])
        ...
        ask_price_list_TSLA2 = pd.merge(left=ask_price_list_TSLA2, right=volume_TSLA2, left_index=True, right_index=True)
        ...
        for i in ['Open_TSLA', 'High_TSLA','Low_TSLA','Close_TSLA','midpoint_TSLA']:
            ...
            ask_price_list_TSLA2 = pd.merge(left=ask_price_list_TSLA2, right=ask_price_list_temp_TSLA, left_index=True, right_index=True)
        
        ask_price_list_TSLA = ask_price_list_TSLA2.ffill()
        ask_price_list_TSLA = ask_price_list_TSLA.rename(columns={"open":"open_TSLA", "high":"high_TSLA", "low":"low_TSLA", "close":"close_TSLA", "volume":"volume_TSLA"})
    ...
    ask_price_list4 = pd.merge(left=ask_price_list4, right=ask_price_list_TSLA, left_index=True, right_index=True)
...
async def create_model(data):
    ...
    data = await trade_data_handler(data)
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        catboost_class_buy, catboost_class_sell, catboost_class_buy1, catboost_class_sell1, catboost_class_buy2, catboost_class_sell2 = executor.map(make_model, x_list, symbol_list, side_list)


    """
    #AAPL BUY SIDE
    catboost_class_buy_AAPL = make_model(dataset_AAPL[-(len(y_AAPL)):], y_AAPL, symbol="AAPL", side="buy")
    y_pred_test_AAPL = catboost_class_buy_AAPL.predict(last_input)
    CatBoost_pred_AAPL = int(y_pred_test_AAPL[-1:])
    previous_CatBoost = int(y_pred_test_AAPL[-2:-1])
    print("last AAPL CatBoost_buy output: ", CatBoost_pred_AAPL)

    if int(CatBoost_pred_AAPL) == 1:
        ...
        for i in range(0,2):
            ...
            limit_order(symbol="AAPL", 
                        ...
                        loss_limit_multiplier = 2.1,
                        qty = 100 + i,
                        inventory_risk = get_inventory_risk(symbol="AAPL")
                        )
    ...
    #AAPL SELL SIDE
    catboost_class_sell_AAPL = make_model(dataset_AAPL[-(len(y_AAPL_sell)):], y_AAPL_sell, symbol="AAPL", side="sell")
    y_pred_test_AAPL_sell = catboost_class_sell_AAPL.predict(last_input)
    CatBoost_pred_AAPL_sell = int(y_pred_test_AAPL_sell[-1:])
    previous_CatBoost_sell = int(y_pred_test_AAPL_sell[-2:-1])
    print("last AAPL CatBoost_buy output: ", CatBoost_pred_AAPL_sell)

    if int(CatBoost_pred_AAPL_sell) == 1:
...
async def get_trade(data):
    # Pause here and come back to g() when f() is ready
    CatBoost_pred, previous_CatBoost, model = await create_model(data)
